...
def check_node_overlap(path_ppi, path_file2check):
    df_ppi = pd.read_csv(path_ppi,sep="\t", encoding="latin-1",names = ["end1", "end2"], usecols=[0,2]).drop_duplicates(ignore_index=True)
    df_2check = pd.read_csv(path_file2check, sep="\t", names = ["end1", "end2"], usecols=[0,2])
    df_ppi["index"], df_2check["index"] = df_ppi.index, df_2check.index
    df_ppi['key'] = df_ppi[["end1", "end2"]].apply(lambda row: tuple(sorted(row)), axis=1)
    df_2check['key'] = df_2check[["end1", "end2"]].apply(lambda row: tuple(sorted(row)), axis=1)
    overlap = pd.merge(df_ppi, df_2check, how='inner', on="key")
    return overlap

def check_rel_distribution():
    pass
