...
def create_splits(dataset, base_split=[.8,.1,.1], train_split: Optional[List[float]] = [.8, .2]):
  ...
  This function takes a dataset of shape (n,3), where each row corresponds to a
  tuple (h,r,t) and splits it into four splits:
  * first, a train, a validation, and a test split are created. It is ensured
    that train contains every node at least once.
  * second, train is split again, into a message passing and a supervision set.
    Again, message passing contains each node at least once.
    
  Args:
    dataset: array of n tuples (h,r,t)
    base_split: the split ratios to create train, validation, and test
    train_split: the split ratios to create message passing and supervision from
      train
  
  Returns:
    A tuple of the four splits: message passing, supervision, validation, and 
    test.
  """
...
def df2nx(
  ...
  creator = nx.MultiDiGraph(), connection_policy=nx.weakly_connected_components
):
  """
